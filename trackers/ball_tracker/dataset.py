...
class BallTrajectoryDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.frame_arr is not None:
            ...
            data_idx = self.data_dict['id'][idx]
            coor_pred = self.data_dict['coor_pred'][idx]
            inpaint = self.data_dict['inpaint_mask'][idx].reshape(-1, 1)
            ...
            coor_pred = self.data_dict['coor_pred'][idx]
            vis = self.data_dict['vis'][idx] 
            vis_pred = self.data_dict['pred_vis'][idx] 
            inpaint = self.data_dict['inpaint_mask'][idx] 
            w, h = self.img_config['img_shape'][data_idx[0][0]]
